...
if ( !class_exists( 'BP_Better_Messages_Functions' ) ):
    ...
    class BP_Better_Messages_Functions
    {
        ...
        public function is_thread_moderator($user_id, $thread_id){
            ...
            SELECT COUNT(*) FROM `{$wpdb->base_prefix}bp_messages_recipients` 
            WHERE `user_id` = %d 
            AND `thread_id` = %d
            AND `sender_only` = '1'
            ", $user_id, $thread_id));
        ...
        public function get_participants( $thread_id )
        {
            ...
            foreach ( $recipients as $recipient ) {
                ...
                if($user->ID != get_current_user_id()) $participants[ 'links' ][] = '<a href="' . bp_core_get_userlink( $recipient->user_id, false, true ) . '" class="user">' . BP_Better_Messages_Functions()->get_avatar( $recipient->user_id, 20 ) . $user->display_name . '</a>';
