...
function weu_smtp_config_page() {
		...
		if(isset($_POST['submit_edit'])){
			...
			if ( ! isset( $_POST['edit_smtp_nonce'] ) || !wp_verify_nonce( $_POST['edit_smtp_nonce'], 'edit_smtp' ))
			{
				...
				foreach ( $myrows as $user ){
					...
					echo '<tr><th>Type of Encription : <font color="red">*</font> &nbsp;</th>
					<td style="width: 170px"><input type="radio" name="weu_smtp_smtpsecure" value="" '; if($user->smtp_smtpsecure=='') echo 'checked'; echo '> None &nbsp;</td>
					<td style="width: 170px"><input type="radio" name="weu_smtp_smtpsecure" value="ssl" '; if($user->smtp_smtpsecure =='ssl') echo 'checked'; echo '> SSL </td>
					<td style="width: 170px"><input type="radio" name="weu_smtp_smtpsecure" value="tls" '; if($user->smtp_smtpsecure=='tls')  echo'checked'; echo '> TLS </td></tr>';
			...
			<th>Type of Encription : <font color="red">*</font> &nbsp;</th>
	...
	if(isset($_POST['submit-delete-smtp']))
	{
		if ( ! isset( $_POST['delete_smtp_nonce'] ))
		{
			...
			$mylink = $wpdb->delete($table_name, array('conf_id' => $local_weu_sent_id), array('%d'));
			if ( null !== $mylink ) { 
